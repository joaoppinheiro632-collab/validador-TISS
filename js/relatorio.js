...
// agora). Mas pra "guiasComInconsistencia" isso seria errado: se o
// Validador pegou 1 inconsistência na primeira validação e o usuário
// corrigiu antes de baixar de novo, a versão mais recente já mostra 0 — e
// perderíamos o crédito do que foi prevenido. Por isso usa o PICO (maior
// valor já visto pra aquele lote hoje), não a última versão.
export function getResumoAutomatico(historico) {
    const porLote = new Map();
    historico.forEach(r => {
        const atual = porLote.get(r.lote);
        if (!atual) {
            porLote.set(r.lote, { ...r, guiasComInconsistenciaPico: r.guiasComInconsistencia || 0 });
            return;
        }
    ...
    const totalGuiasComInconsistencia = registros.reduce((acc, r) => acc + (r.guiasComInconsistenciaPico || 0), 0);
