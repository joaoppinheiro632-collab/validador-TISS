...
class App {
    ...
    registrarTransmissaoAtual(origem) {
        ...
        registrarTransmissao({
            ...
            // "Erros prevenidos": conta que o Validador pegou pelo menos um
            // erro crítico nessa guia, antes do envio — não é sobre ter
            // editado o texto do XML. Isso importa porque, se o mesmo lote
            // for baixado de novo depois de corrigido, a versão nova já não
            // tem mais erro nenhum; getResumoAutomatico() guarda o PICO desse
            // número por lote (ver relatorio.js), pra não perder o crédito
            // do que foi pego só porque depois foi corrigido.
            guiasComErro: this.validationResults.guias.filter(g => g.erros > 0).length,
